...
try {
  ...
  Get-ChildItem -file -Recurse . |
    Where-Object { $_.Name -match ".*\.cpp" } |
    ForEach-Object -Process {
      $target = (Get-Item $_).Basename
      Write-Header "cleaning $target"
      Remove-Item "$target.obj", "$target.dll", "$target.exp", `
        "$target.lib", "$target.winmd" -ErrorAction SilentlyContinue
      Write-Header "compiling $_"
      cl /nologo /EHsc /Gm- /GR- /W4 /LD `
        /D_CRT_SECURE_NO_WARNINGS=1 $_
      if (-not $?) {
